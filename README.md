...
> **실행/연결 정본: [docs/RUN.md](docs/RUN.md)** — 연결 모델·`.env`·래퍼 스크립트·문제해결을 한 곳에. 설정은 `.env` 하나가 단일 출처(`cp .env.example .env` 후 시뮬 IP만 수정).

**실행 순서가 중요하다.** 브릿지를 `auto`로 **먼저** 띄운 뒤 시뮬을 (재)시작해야 `/init` 핸드셰이크가 닿는다.

```bash
# 터미널 1 — HTTP 브릿지 (설정은 .env 자동 적용)
scripts/run_bridge.sh --mode auto

# 터미널 2 — (Windows) 시뮬레이터 시작/재시작 → /init 핸드셰이크
#            시뮬 UI 엔드포인트 = "내우분투IP:5000"  (hostname -I 로 IP 확인)

# 터미널 3 — 전체 자율주행 스택 (lidar + 인지 + A* + APF + 컨트롤러)
scripts/run_stack.sh --route A
