...
echo "============================================================"
echo "log dir    : /home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942"
...
run_step() {
  ...
  echo "Command: $*"
  echo
  "$@" 2>&1 | tee "/home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942/$name.log"
  local code="${PIPESTATUS[0]}"
  if [[ "$code" -ne 0 ]]; then
    echo
    echo "[ERROR] $name failed with exit code $code"
    echo "로그: /home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942/$name.log"
    exec bash
  fi
...
wait_for_bridge() {
  ...
  for _ in $(seq 1 60); do
    if command -v curl >/dev/null 2>&1 && curl -fsS "http://127.0.0.1:5000/health" >"/home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942/bridge_health.json" 2>/dev/null; then
      echo "[OK] bridge health: $(cat "/home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942/bridge_health.json")"
      return 0
    fi
    sleep 0.5
  done
...
request_simulator_reset() {
  ...
  for i in 1 2 3; do
    timeout 6s ros2 topic pub --once /tank/episode/control std_msgs/msg/String "{data: 'reset'}"       2>&1 | tee "/home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942/reset_attempt_${i}.log" || true
    sleep 0.5
  done
}
...
echo "로그: /home/tankcc/tankcc/logs/scenario2_terminator_20260709_092942"
exec bash
