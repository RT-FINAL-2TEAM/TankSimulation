...
UBUNTU_SERVER = os.environ.get("UBUNTU_SERVER", "http://192.168.0.15:5030")
PROXY_HOST = os.environ.get("PROXY_HOST", "127.0.0.1")
PROXY_PORT = int(os.environ.get("PROXY_PORT", "5030"))
...
def proxy(path):
    ...
    try:
        ...
        print(f"[PROXY][ERROR] {request.method} /{path} -> {target_url}: {e}")
...
if __name__ == "__main__":
    ...
    if UBUNTU_SERVER.rstrip("/") == f"http://{PROXY_HOST}:{PROXY_PORT}":
        print("[PROXY][WARN] forwarding target is the same as the proxy listener")
