        ...
        <script>
            ...
            function updateLeftPanel(state) {
                ...
                if (activeTab === "ai") {
                    ...
                    const entry = values.length ? values[values.length - 1] : null;
                    if (!entry) {
                    ...
                    const conf = res.confidence || entry.confidence;
                    const summ = res.summary || entry.summary;
                    const reason = res.decision_reason || entry.decision_reason;
                    byId("leftContent").innerHTML = renderReadouts([
                        { label: "모델", value: safe(entry.model, "-") },
                        { label: "추천 루트", value: safe(sel, "-") },
                        { label: "위험도 A/B", value: `${safe(rl.A, "-")} / ${safe(rl.B, "-")}` },
                        { label: "확신도", value: safe(conf, "-") },
                        { label: "요약", value: safe(summ, "-") },
                        { label: "판단 근거", value: safe(reason, "-") },
