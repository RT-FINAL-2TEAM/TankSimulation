...
class LLMReporter:
    ...
    def build_prompt(self, comparison_data: dict) -> str:
        ...
        - selected_route는 두 루트 중 전차가 실제로 주행할 더 안전한 루트를 선택한다. 위험도가 낮은 쪽을 고른다.
        - risk_level은 반드시 {{"A": "<값>", "B": "<값>"}} 형태여야 한다. 키는 반드시 "A"와 "B"이고, 값은 "low"/"medium"/"high"/"critical" 중 하나다. 예: {{"A": "low", "B": "high"}}
        - confidence 값은 "low", "medium", "high" 중 하나다.
        - speed_policy 값은 "slow", "medium", "fast" 중 하나다.
        - key_risks는 반드시 {{"A": ["..."], "B": ["..."]}} 형태여야 한다. 배열 하나만 쓰지 마라. 예: {{"A": ["적군 1명 근접"], "B": ["충돌 1회, 적군 3명"]}}
    ...
    def generate_route_decision(self, comparison_data: dict) -> dict:
        ...
        if not validated_ok:
            # JSON 파싱 자체가 실패한 경우만 fallback 사용.
            # 파싱은 됐지만 스키마가 불완전한 경우엔 validate_and_fix_result가 보정.
