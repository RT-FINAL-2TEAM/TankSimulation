...
STEERING_KD = env_float("TANK_CONTROLLER_STEERING_KD", 0.15)
STRAIGHT_WS_WEIGHT = env_float("TANK_CONTROLLER_STRAIGHT_WS_WEIGHT", 0.9)
TURN_WS_WEIGHT = env_float("TANK_CONTROLLER_TURN_WS_WEIGHT", 0.4)
ROTATE_IN_PLACE_ANGLE_DEG = env_float("TANK_CONTROLLER_ROTATE_IN_PLACE_ANGLE_DEG", 75.0)
CRAWL_PIVOT_ANGLE_DEG = env_float("TANK_CONTROLLER_CRAWL_PIVOT_ANGLE_DEG", 90.0)
CRAWL_PIVOT_WS_WEIGHT = env_float("TANK_CONTROLLER_CRAWL_PIVOT_WS_WEIGHT", 0.1)
CRAWL_TURN_WS_WEIGHT = env_float("TANK_CONTROLLER_CRAWL_TURN_WS_WEIGHT", 0.2)
SLOWDOWN_ANGLE_DEG = env_float("TANK_CONTROLLER_SLOWDOWN_ANGLE_DEG", 30.0)
STOP_DISTANCE = env_float("TANK_CONTROLLER_STOP_DISTANCE", 12.0)
...
# planner가 /tank/planner/path_points에 발행하는 곡선/감속 profile을 제어기에 전달한다.
# potential_field의 compute_desired_motion과는 연결하지 않는다.
TOPIC_PATH_POINTS = os.environ.get("TANK_TOPIC_PATH_POINTS", "/tank/planner/path_points")
ENABLE_PLANNER_SPEED_PROFILE = env_bool("TANK_CONTROLLER_ENABLE_PLANNER_SPEED_PROFILE", True)
PLANNER_SPEED_PROFILE_TTL_SEC = env_float("TANK_CONTROLLER_PLANNER_SPEED_PROFILE_TTL_SEC", 3.0)
PLANNER_GOAL_STOP_DISTANCE_M = env_float("TANK_CONTROLLER_PLANNER_GOAL_STOP_DISTANCE_M", 3.0)
